# DIY Система інвертора + акумулятора + моніторинг та захист від перерозрядки з XH-M609, ESP32 та PZEM-017

Цей проект демонструє систему DIY інвертора, акумулятора та моніторингу із захистом від перерозрядки. Використовується контролер акумулятора XH-M609, DC-DC стабілізатор і енергомонітор PZEM-017, підключений до ESP32 через перетворювач RS485-to-TTL. Система включає реле, яке керує живленням інвертора, забезпечуючи автоматичне вмикання та вимикання на основі стану акумулятора.

## Огляд системи

- **Контролер акумулятора XH-M609**: Керує зарядом і розрядом акумулятора, а також забезпечує захист від перерозрядки. Керує реле, яке живить інвертор.
- **DC-DC стабілізатор**: Перетворює напругу на стабільні 12В для керування реле.
- **PZEM-017**: Моніторить електричні параметри, такі як струм, напруга та потужність. Підключений до ESP32 для збереження даних і моніторингу.
- **ESP32**: Збирає дані з PZEM-017 та інтегрується з ESPHome для віддаленого моніторингу та керування.
- **Реле**: Вмикає/вимикає інвертор залежно від стану акумулятора, як це контролюється XH-M609.

## Особливості

- **Керування реле** на основі стану акумулятора (автоматичне вмикання/вимикання інвертора).
- **Енергомоніторинг** за допомогою PZEM-017.
- **Інтеграція з ESPHome** для віддаленого моніторингу та керування за допомогою настроюваних конфігурацій.
- **Modbus комунікація** для налаштування діапазону струму системи на основі використовуваного шунта.

## Схема підключень

![Схема підключень](./wiring.png)

## Фрагмент коду для конфігурації ESPHome

Наступний код YAML використовує бібліотеку pzemdc для ESPHome і забезпечує зв'язок з PZEM-017:
...
sensor:
  ...
  - platform: pzemdc
    address: 0x01 # Адреса пристрою pzemdc
    voltage:
      name: "DC-Voltage"
      id: DC_Voltage
      accuracy_decimals: 3
  ...
  # Датчик відсотка заряду акумулятора
  - platform: template
    name: "Battery SOC %"
...
Наступний код YAML використовує лише Modbus для зв'язку з PZEM-017 замість бібліотеки pzemdc для ESPHome. Ось приклад того, як змінити діапазон струму шунта на 300A (може бути змінено залежно від використовуваного шунта):
...
modbus_controller:
  - id: modbus_device
    address: 0x01   # Адреса Modbus пристрою на шині
    modbus_id: modbus1
    setup_priority: -10
...
button:
  - platform: template
    ...
    on_press:
      then:
        - lambda: |-
            std::vector<uint16_t> current_range_payload = {0x0003};  // 300A значення, змінити для вашого шунта
  ...
  - platform: template
    ...
    on_press:
      then:
        - lambda: |-
            std::vector<uint16_t> current_range_payload = {0x01};  // команда зміни адреси
